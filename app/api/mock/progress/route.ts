  ...
  try {
    ...
    const activeGoalsProgress = activeGoals.map((goal) => {
      ...
      return {
        ...
        color: goal.calendar.color,
        progress: goal.progress.completionRate,
        tasksCompleted: goal.progress.tasksCompleted,
        totalTasks: goal.progress.totalTasksScheduled,
    ...
    const milestoneDelta = activeGoals
      .flatMap((goal) =>
        (goal.plan?.milestones || [])
          ...
          .map((milestone) => {
            const actualProgress = goal.progress.completionRate
